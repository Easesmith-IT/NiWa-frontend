...
const getStatusBadge = (status: string) => {
  ...
  switch (normalized) {
    case "APPROVED":
      return (
        <span className="inline-flex items-center gap-1.5 rounded-full bg-[#EDF8F3] px-2.5 py-0.5 text-xs font-semibold text-[#16803C]">
...
const getCategoryBadge = (category: string) => {
  ...
  switch (cat) {
    case "MARKETING":
      return (
        <span className="rounded-full bg-[#F4F4F5] border border-[#E4E4E7] px-2.5 py-0.5 text-[11px] font-medium text-foreground">
          Marketing
        </span>
      );
    case "UTILITY":
      return (
        <span className="rounded-full bg-[#EDF8F3] border border-emerald-200 px-2.5 py-0.5 text-[11px] font-medium text-[#176B4D]">
          Utility
        </span>
      );
    case "AUTHENTICATION":
    ...
    default:
      return (
        <span className="rounded-full bg-[#F4F4F5] px-2.5 py-0.5 text-[11px] font-medium text-muted-foreground">
          {category}
        </span>
      );
  }
...
export function WhatsAppTemplateCard({ template }: WhatsAppTemplateCardProps) {
  ...
  const renderFormattedBody = (text: string) => {
    ...
    return parts.map((part, idx) => {
      if (/^{{\s*\w+\s*}}$/.test(part)) {
        return (
          <span
            className="mx-0.5 inline-flex items-center rounded bg-[#EDF8F3] border border-emerald-200 px-1 py-0.2 text-xs font-medium text-[#176B4D]"
            key={idx}
          >
            {part}
          </span>
  ...
  return (
    <div className="flex flex-col overflow-hidden rounded-lg border border-[#E4E4E7] bg-white shadow-subtle transition-colors hover:border-[#D4D4D8] lg:flex-row">
      {/* Left Column */}
        ...
        <div className="space-y-2.5">
          <div className="flex flex-wrap items-center justify-between gap-2">
            <div className="flex items-center gap-2">
              ...
              <span className="rounded-md border border-[#E4E4E7] bg-[#FAFAFA] px-2 py-0.5 font-mono text-[11px] font-medium text-muted-foreground">
                {template.language}
              </span>
            </div>
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            {variables.length > 0 ? (
              <span className="rounded-full bg-[#FAFAFA] border border-[#E4E4E7] px-2.5 py-0.5 text-[11px] font-medium text-muted-foreground">
                {variables.length} variable{variables.length === 1 ? "" : "s"} ({variables.map((v) => `{{${v}}}`).join(", ")})
              </span>
            ) : null}
            {template.isSendable ? (
              <span className="rounded-full bg-[#EDF8F3] px-2.5 py-0.5 text-[11px] font-semibold text-[#16803C]">
                Ready to Send
              </span>
            ) : null}
          </div>
      ...
      {/* Right Canvas */}
      <div className="w-full border-t border-[#E4E4E7] bg-[#F7F8FA] p-5 lg:w-[360px] lg:border-l lg:border-t-0 flex flex-col justify-center">
