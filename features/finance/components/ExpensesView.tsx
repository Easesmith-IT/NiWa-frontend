...
import { financeApi, Expense } from "lib/api/finance-api";
...
export function ExpensesView() {
  ...
  const handleCreateSubmit = (e: React.FormEvent) => {
    ...
    createMutation.mutate({
      ...
      taxConfigurationId: formData.taxConfigurationId || undefined,
          ...
          <div className="w-full max-w-xl rounded-xl bg-white p-6 shadow-xl max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleCreateSubmit} className="mt-4 space-y-4">
              ...
              <div className="grid grid-cols-3 gap-4">
                ...
                <div>
                  <label className="text-xs font-medium text-slate-700">Tax Code / Rate</label>
                  <select
                    value={formData.taxConfigurationId || (formData.taxRatePercent ? `manual-${formData.taxRatePercent}` : "none")}
                    onChange={(e) => {
                      ...
                      if (val === "none") {
                        ...
                        const matchedConfig = activeTaxConfigs.find((c) => c._id === val);
                        if (matchedConfig) {
                          setFormData({
                            ...formData,
                            taxConfigurationId: matchedConfig._id,
                            taxCode: matchedConfig.taxCode,
                            taxRatePercent: matchedConfig.rate,
                          });
                        } else if (val.startsWith("manual-")) {
                          const rate = parseFloat(val.replace("manual-", "")) || 0;
                          setFormData({
                            ...formData,
                            taxConfigurationId: undefined,
                            taxCode: undefined,
          ...
          <div className="w-full max-w-lg rounded-xl bg-white p-6 shadow-xl">
            ...
            <div className="mt-4 space-y-3 text-xs text-slate-600">
              ...
              <div className="rounded-lg bg-slate-50 p-3 space-y-1">
                ...
                <div className="flex justify-between">
                  <span>
                    Tax {selectedExpense.taxCode ? `(${selectedExpense.taxCode})` : selectedExpense.taxRatePercent ? `(${selectedExpense.taxRatePercent}%)` : ""}:
                  </span>
